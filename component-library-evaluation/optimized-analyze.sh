...
# Enhanced Bundle Analysis Script with Import Optimizations
# Tests both original and optimized import patterns for accurate comparison

echo "🚀 Starting enhanced bundle size analysis with import optimizations..."

RESULTS_FILE="component-library-evaluation/optimized-results.md"
...
# Test 1: Baseline (Current MUI with optimized imports - already done)
build_and_measure "Baseline - Material UI (Optimized Imports)" "Current implementation using optimized @mui/material imports from individual component paths."
...
# Test 3: Headless UI (already optimized)
...
build_and_measure "Headless UI (Optimized)" "Uses @headlessui/react with optimized tree-shaking friendly imports."
...
echo "| Material UI (Optimized) | ${baseline_kb} KB | Baseline | Individual component imports | Tree-shaking friendly |" >> $RESULTS_FILE

echo ""
echo "📊 Analysis complete! Results saved to $RESULTS_FILE"
echo ""
echo "Key findings:"
echo "- Material UI baseline (optimized imports): ${baseline_kb} KB gzipped"
echo "- Import optimizations can provide additional bundle size reductions"
echo "- Tree-shaking effectiveness varies by library"